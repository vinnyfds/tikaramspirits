...
export default function ProductStoryScroll({
  ...
  useLayoutEffect(() => {
    ...
    // Only run on desktop / larger tablets
    ...
    const ctx = gsap.context(() => {
      ...
      // Initial state: all cards hidden and positioned lower for parallax entry
      gsap.set(cards, { autoAlpha: 0, y: 20, pointerEvents: 'none' })
      // First card visible and centered
      gsap.set(cards[0], { autoAlpha: 1, y: 0, pointerEvents: 'auto' })
      // Initial state for sensory notes (hidden and offset)
      gsap.set('[data-sensory-note]', { autoAlpha: 0, x: -5 })
      ...
      // Sequential cross-fade transitions with parallax
      cards.forEach((card, index) => {
        ...
        // Simultaneous fade out previous (with exit parallax) and fade in current (with entry parallax)
        // Use sequential positioning: +=1 for each transition
        masterTimeline.to(
          prev,
          {
            autoAlpha: 0,
          ...
          {
            autoAlpha: 0,
            y: 20, // Start from lower position
            pointerEvents: 'none',
          },
          {
            autoAlpha: 1,
            y: 0, // Move to centered position
            pointerEvents: 'auto',
            duration: 1,
  ...
  return (
    <section ref={sectionRef} className="relative bg-white">
      <div
        ...
        className="max-w-[1440px] mx-auto min-h-screen grid grid-cols-1 lg:grid-cols-[minmax(0,0.75fr)_1px_minmax(0,1.25fr)]"
      >
        ...
        <div className="relative flex items-center justify-center py-24">
          <div ref={cardsRef} className="relative h-[70vh] w-full max-w-xl mx-auto overflow-hidden">
            {storyCards.map((card) => (
              <article
                key={card.id}
                data-card={card.id}
                className="absolute left-1/2 top-1/2 w-full -translate-x-1/2 -translate-y-1/2 opacity-0 pointer-events-none"
              >
                <div className="bg-white shadow-level-1 rounded-soft px-8 py-10 lg:px-10 lg:py-12">
                  ...
                  {card.id === 'overview' && (
                    <>
                      ...
                      {/* BUY NOW Button */}
                      <div className="mt-16">
                        <Link href="/store-locator" className="inline-block w-full sm:w-auto">
